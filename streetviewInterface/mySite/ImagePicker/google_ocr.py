...
def google_ocr(api_key, streetviewImage):
    ...
    if response.status_code != 200 or response.json().get('error'):
        ...
        for idx, resp in enumerate(responses): # response.json()['responses'][i] corresponds to ith image TODO: skip first image
            ...
            locale = resp['textAnnotations'][0][0]['locale']
            for annotation in resp['textAnnotations']: # skip first annotation? seems to be a composite
                # bounding box
