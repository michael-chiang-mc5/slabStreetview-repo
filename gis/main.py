...
mappoint_path = '../streetviewInterface/mySite/output/MapPoints.csv'
...
num_mapPoint = sum(1 for line in open(mappoint_path))
mappoint_pk_store = num_mapPoint * [None]
mappoint_zone_store = num_mapPoint * [None]

count = 0
with open(mappoint_path) as f:
    for line in f:
        tmp = line.split(',')
        pk = int(tmp[0])
        lat = float(tmp[1])
        ...
        if result[0] == float('inf'):
            mappoint_pk_store[count] = pk
            mappoint_zone_store[count] = 'U'
        else:
            mappoint_pk_store[count] = pk
            mappoint_zone_store[count] = zoning_store[result[1]]
        count += 1
t3 = time.time()
